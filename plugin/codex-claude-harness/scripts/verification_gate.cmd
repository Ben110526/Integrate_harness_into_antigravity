...
:; case "$python_runtime" in /*) if [ -f "$python_runtime" ] && [ -x "$python_runtime" ]; then exec "$python_runtime" "$script_dir/verification_gate.py" "$1"; fi ;; esac
:; if [ "$1" = "stop" ]; then printf '%s\n' '{"decision":"allow","reason":"Verification gate is unavailable because Python 3.8 or newer is not installed; allowing stop to avoid a lockout."}'; else printf '%s\n' '{}'; fi
:; exit 0
@echo off
setlocal
...
set "harness_python_script=%~dp0verification_gate.py"
...
if /I "%~1"=="stop" (
  echo {"decision":"allow","reason":"Verification gate is unavailable because Python 3.8 or newer is not installed; allowing stop to avoid a lockout."}
) else (
  echo {}
)
exit /b 0
