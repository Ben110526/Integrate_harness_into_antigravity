...
1. Confirm every supplied `AC-*` against the actual diff and behavior; preserve its ID and record concrete evidence/check and result. An acceptance criterion without evidence is `[UNRESOLVED]`, not complete.
...
6. Report ready/not-ready, a compact `AC-ID -> evidence/check -> result` matrix, remaining risks, and the exact next action if one is needed. Verify cited source paths and symbols; label non-established claims `[HYPOTHESIS]`, `[ASSUMPTION]`, or `[UNRESOLVED]`.
