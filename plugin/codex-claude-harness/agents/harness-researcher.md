...
- Return a structured **Behavior Map** whenever tracing system behavior, diagnosing issues, or preparing implementation: entry point → call sites → core behavior → outputs/side effects → tests; invariants; competing hypotheses and the cheapest discriminating check; proposed edit files versus read-only dependencies. State the inspected revision and any relevant uncommitted changes. Read the smallest slice that resolves the question; do not measure understanding by file count or summary length.
